...
public class DatabaseMock_OLD {
	...
	public DatabaseMock_OLD() {
		 ...
		 */
		List<Esame> esamiIngegneriaInformaticaUnibo = new ArrayList<>();
		try {
			DataSource dataSource = new DataSource(DataSource.DB2);
			Connection connection = dataSource.getConnection();
			
			Statement statement = connection.createStatement();
			ResultSet rs = statement.executeQuery("SELECT e.* from esami e join piani_formativi p on e.idPianoFormativo = p.id where p.id = 1");
			...
			while(rs.next()) {
				...
				esamiIngegneriaInformaticaUnibo.add(e);
			}
		...
		Collections.sort(esamiIngegneriaInformaticaUnibo, Comparator.comparingInt(Esame::getAnno).thenComparingInt(Esame::getPeriodo).thenComparingInt(Esame::getCFU).thenComparing(Esame::getNome));
		pianoFormativo.setEsami(esamiIngegneriaInformaticaUnibo);
		pianoFormativo2.setEsami(esamiIngegneriaInformaticaUnibo);
		pianoFormativo3.setEsami(esamiIngegneriaInformaticaUnibo);
		pianoFormativo4.setEsami(esamiIngegneriaInformaticaUnibo);
		pianoFormativo5.setEsami(esamiIngegneriaInformaticaUnibo);
